...
@onready var outputlabel: Label = $"../MarginContainer/Label"
...
# Turret should rotate to look at this position
...
func rotate_and_elevate(delta: float, current_target:Vector3) -> void:
	...
	# For testing purposes, position the projection indicator.
	# This will appear "beneath" (or above) the target, on the
	# turret's plane of rotation.
	...
	if outputlabel != null:
		...
		if round(rad_to_deg(x_angle)) == 0:
			...
			elevation_text = "down"
		outputlabel.text = "%d degrees %s\n%d degrees %s" % [rad_to_deg(y_angle), rotation_text, rad_to_deg(x_angle), elevation_text]
